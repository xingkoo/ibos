...
TreeCategory.prototype.add = function(data, setting){
	...
	$.post(url, data, function(res){
		if(res.isSuccess || res.IsSuccess) {
			// 获取父树节点
			var pTreeNode = _this.tree.getNodeByParam("id", data.pid);
			// 使用返回的信息构建新的树节点
			var treeNode = $.extend({
				name: data.name,
				pid: data.pid,
			...
			// 当返回内容中包含url时，作用链接处理
			if(res.url) {
				treeNode.url = res.url;
				treeNode.target = res.target || "_self";
			}
...
TreeCategory.prototype.moveup = function(cid, setting){
	...
	if(treeNode) {
		...
		// 当存在前一树节点时，上移才有意义
		if(prevTreeNode){
			$.post(url, { cateop: "move", type: "moveup", catid: cid, pid: treeNode.pid }, function(res){
				if(res.isSuccess || res.IsSuccess) {
...
TreeCategory.prototype.movedown = function(cid, setting){
	...
	if(treeNode) {
		...
		// 当存在前一树节点时，上移才有意义
		if(nextTreeNode){
			$.post(url, { cateop: "move", type: "movedown", catid: cid, pid: treeNode.pid }, function(res){
				if(res.isSuccess || res.IsSuccess) {
 ...
 */
SideTreeCategory.prototype._createCateOptions = function(selected) {
	...
	var _join = function (nodes, tpl) {
		...
		for(var i = 0; i < nodes.length; i++) {
			tpl += '<option value="' + nodes[i].catid + '"' + (nodes[i].catid == selected ? "selected" : "") +'>' + 

...
Ibos.app = (function(){
	 ...
	 * @return {String}
	 */
	app.getStaticUrl = function(url, mod){
		var baseUrl;
		if(mod) {
			baseUrl = this.g("assets")[mod] || "";
		} else {
			baseUrl = this.g("STATIC_URL");
		}
		return baseUrl + (url || "");
	};
	 ...
	 */
	app.guide = function(id, callback){
		$.ajax({
			url: Ibos.app.url("main/default/moduleGuide"),
			type: "post",
			data: {id: id, op: 'checkHasGuide'},
			dataType: "json",
	...
	app.finishGuide = function(id) {
		$.post(Ibos.app.url("main/default/moduleGuide"), {id: id, op: 'finishGuide'});
	};

		// Ibos.local.remove('guided');
...
(function(window, $){
	...
	(function(){
		...
		/**
		 * 图片延迟加载, 地址放置到data-src属性中
		 * @method delayImage
		 * @param {Jquery}   [$ctx = $(document)]    要作用的图片上下文
		 * @param {Function} [callback]              回调函数，每一张图片加载完时回调，本图片节点作为参数传递
		 */
		U.delayImage = function($ctx, callback){
			$("[data-src]", $ctx).each(function(){
				var $img = $(this),
					$parent = $img.parent();
				$parent.waiting(null, "mini");
		...
		/**
		 * 获取上下文内有name属性的节点的value值，
		 * 必要时可自定义要获取的属性名及值名
		 * @method getFormData
		 * @deprecated
		 * @param  {Jquery} $context [description]
		 * @param  {[type]} adapter  [description]
		 * @return {[type]}          [description]
		 */
		U.getFormData = function($context, adapter){
			var data = {},
				props = $.extend({
					name: "name",
					value: "value"
			...
			$("[" + props.name + "]", $context).each(function(){
				var $control = $(this),
					name = $control.attr(props.name),
					value = props.value === "value" ? $control.val() : $control.attr(props.value);
		 ...
		 */
		U.lang = function(langNS, data) {
			var lang = L;
			if (!lang) {
				throw Error("U.lang: 语言包未定义");
			}
			langNS = (langNS || "").split(".");
			...
			return data ? (data === true ? $.template(lang, L) : $.template(lang, data)) : lang;
		}
	...
	(function(){
		...
		$.extend(Ui, {
			 ...
			 * @deprecated
			 * @todo  形式不好，大概需要重写
			 * @param {String} name 要触发联动的checkbox的name属性
			 * @param {Jquery} context 上下文
			 * @return {Object} 当前调用对象
			 */
			setLinkage: function(name, $context, callback){
				return;
				if(!$context || !$context.length){
					$context = $(document);
				}
				var leaderSelector = 'input[type="checkbox"][data-name="' + name + '"]',
					memberSelector = 'input[type="checkbox"][name="' + name + '"], input[type="checkbox"][data-check="' + name + '"]',
					// 表示当前复选是否选中的状态
					status, 
					_getChecked = function(){
						return $(memberSelector, $context).filter(":checked");
					},
					//当复选框由Label对象初始化时, 同步刷新其对应的Label对象
					_refresh = function($elem){
						var labelData = $elem.data("label");
						if(labelData) {
							labelData.refresh();
						}
					};
				// 选中或取消全选框时，同步对应子复选框的状态
				$context.on('change', leaderSelector, function(){

					var $cb = $(this),
						$member = $(memberSelector, $context);

					status = $cb.prop('checked');

					$member.each(function(){
						_refresh($(this).prop("checked", status).trigger("change"));
					})

					_call(callback, _getChecked());
				})
				// 取消子复选框时，同时取消全选
				.on('change', memberSelector, function(){
					var $cb = $(this),
						$leader = $(leaderSelector, $context);
			 ...
			 * @param  {String} winName  窗口名
			 * @param  {String} paramStr 弹窗参数
			 * @return {}          
			 */
			openFullWindow: function(url, winName, paramStr){
				paramStr = paramStr ? paramStr : "menubar=0,toolbar=0,status=0,resizable=1,scrollbars=1";
				paramStr += ",top=0,left=0,height=" + screen.availHeight + ",width=" + screen.availWidth;
		 ...
		 * @return {}
		 */
		Ui.showPmDialog = function(toUid, ajaxOptions, selectOptions){
			var content = '<div style="width: 500px;">' +
				'<form>' +
					'<table class="table table-condensed">' +
						'<tr>' +
							'<td class="xar" width="50">' + U.lang("RECEIVER") + '</td>' +
							'<td>' +
								'<input type="text" name="touid" data-toggle="userSelect">' +
								'<div></div>' +
							'</td>' +
						'</tr>' +
						'<tr>' +
							'<td class="xar vat">' + U.lang("CONTENT") + '</td>' +
							'<td>' +
								'<textarea name="content" rows="5" ></textarea>' +
							'</td>' +
						'</tr>' +
					'</table>' +
					'<input type="hidden" name="formhash" value="' + Ibos.app.g('formHash') + '" />' +
				'</form>' +
			'</div>';
		 ...
		 * @return {[type]}          [description]
		 */
		Ui.showLoginDialog = function(defaults){
			if(typeof $.formValidator === "undefined") {
				U.loadFile(document, {
					id: "css_formvalidate",
					tag: "link",
					rel: "stylesheet",
					href: Ibos.app.getStaticUrl("/js/lib/formValidator/themes/Ibos/style/style.css")
				});

				$.getScript(Ibos.app.getStaticUrl("/js/lib/formValidator/formValidator.packaged.js"), function(){
					fv_scriptSrc = Ibos.app.getStaticUrl("/js/lib/formValidator/formValidator.packaged.js");
					Ui.showLoginDialog(defaults);
				});
				return;
			}

			var tpl = '<form id="ajax_login_form" class="form-horizontal form-compact" style="width: 400px;">' + 
				'<div class="alert alert-main">' + U.lang("LOGIN.TIMEOUT") + '</div>' +
				'<div class="control-group">' +
					'<label class="control-label">' + U.lang("USERNAME") + '</label>' +
						'<div class="controls">' + 
							'<input type="text" name="username" id="ajax_username" value="<%= username %>"/>' +
						'</div>' +
					'</div>' +
				'</div>' +
				'<div class="control-group" style="margin-bottom: 0">' +
					'<label class="control-label">' + U.lang("PASSWORD") + '</label>' +
						'<div class="controls">' + 
							'<input type="password" name="password" id="ajax_password" value="<%= password %>"/>' +
						'</div>' +
					'</div>' +
				'</div>' +
			'</form>';
			...
			Ui.dialog({
				id: "d_login",
				title: false,
				content: $.template(tpl, $.extend({ username: "", password: "" }, defaults)),
				lock: true,
				init: function(){
					// 初始化表单验证
					...
					$("#ajax_username").formValidator({
						onFocus: U.lang("V.INPUT_USERNAME"),
						validatorGroup: "100"
					})
					.regexValidator({
						dataType: "enum",
						regExp: "notempty",
						onError: U.lang("V.INPUT_USERNAME")
					});

					$("#ajax_password").formValidator({ 
						onFocus: U.lang("V.INPUT_POSSWORD"),
						validatorGroup: "100"
					})
					.regexValidator({
						dataType: "enum",
						regExp: "notempty",
						onError: U.lang("V.INPUT_POSSWORD")
					});
					// 按回车键提交
	...
	$.extend(Ibos, {
		...
		date: {
			...
			numberToTime: function(num){
				...
				var hour = intBit > 9 ? intBit : "0" + intBit;
				var minute = parseInt(decBit * 60);
		...
		matchSpell: function(term, text){
			...
			// 优先匹配全拼，匹配成功时返回true，不成功时再匹配首字母拼
			pinyinText = pinyinEngine.toPinyin(text, false, ",")
			pinyinText = typeof pinyinText === 'string' ? pinyinText : pinyinText[0];

			match = (text + "," + pinyinText.toLowerCase()).indexOf(term.toLowerCase()) !== -1;

			// 第一个字符可以从字符串任何位置开始, 找到后记录其位置
			var _matchFirstLetter = function(term, chars) {
				for(var i = 0; i < chars.length; i++) {
					if($.isArray(chars[i])) {
						if(_matchFirstLetter(term, chars[i])) {
							return true;
						}
					} else {
						if(chars[i].charAt(0) === term){
							matchStart = i;
							return true;
						}
					}
			...
			// 第二个字符必须匹配第一个字符的下一字符，以此类推
			var _matchOtherLetter = function(term, chars){
				for(var i = 0; i < chars.length; i++) {
					// 如果是汉字，可能有多种组合，即会出现几个数组的可能，此时需要循环匹配
					// 只要几种组合中，有一个能匹配正确，都返回true;
					if($.isArray(chars[i])) {
						if(_matchOtherLetter(term, chars[i])) {
							return true;
						}
					} else {
						if(chars[matchStart + 1] && chars[matchStart + 1].charAt(0) === term) {
							matchStart++;
							return true
						}
					}
			...
			// 首字母拼写
			if(!match) {
				textChars = pinyinEngine.toPinyin(text, false); 
				termChars = term.split("");
				// 如：晒妹子必须死
				// 匹配 smzbxs | mzbxs | bxs
				// 不匹配 szbxs
				for(var i = 0; i < termChars.length; i++) {
					if(i === 0) {
						match = _matchFirstLetter(termChars[i], textChars);
					} else {
						match = _matchOtherLetter(termChars[i], textChars);
					}
					if(!match) {
						// 只要有一个字符不匹配则马上返回false, 不再继续
						return false;
					}
				}
			}
	...
	$.extend(Ibos, (function(){
		var hasChange = false;
		var ignoreFormChange = function(){
			hasChange = false;
		}
		var monitorEditorChange = function($ctx, callback){
			var addChangeEvent = function(ue){
				ue.addListener("contentChange", function(){
					callback && callback();
				})
			}
			
			if(typeof UE !== "undefined" && !$.isEmptyObject(UE.instants)){	
				for(var i in UE.instants) {
					if(UE.instants[i].isReady) {
						// 当编辑器在监听范围内时， 添加change事件
						if($.contains($ctx[0], UE.instants[i].iframe)) {
							addChangeEvent(UE.instants[i]);
						}
					} else {
						UE.instants[i].addListener("ready", function(){
							if($.contains($ctx[0], UE.instants[i].iframe)) {
								addChangeEvent(UE.instants[i]);
							}
						})
		...
		/**
		 * 检查表单内容是否发生改变，并在离开页面时作对应提示
		 * 在控件上加data-nocheck属性可跳过检查
		 * @method checkFormChange
		 * @for Ibos
		 * @param  {Jquery} $context    上下文环境，一般情况下为要检查的表单
		 * @param  {String} [tip]       离开提示文本
		 * @todo  由于input[type=text]及textarea在散焦后才会触发change，考虑是否用Keydown事件。
		 * @todo  特殊情形 @“百度编辑器”
		 * @return {[type]}          [description]
		 */
		var checkFormChange = function($context, tip, editor){
			tip = tip || "离开页面后，未保存的数据将丢失";
			editor = editor === false ? false : true;

			$context = $context && $context.length ? $context : $(document.body)
			var controlSelector = "input,textarea,select";

			if(editor) {
				monitorEditorChange($context, function(){
					hasChange = true;
				});
			}

			// @Todo: 使用oninput onpropertychange判断会否好些？
			$context.on("change", controlSelector, function(){
				// 拥有特殊属性data-nocheck的，跳过检查。
				var noCheck = typeof $.attr(this, "data-nocheck") !== "undefined";
				!noCheck && (hasChange = true);
			});
		...
		return {
			ignoreFormChange: ignoreFormChange,
			triggerFormChange: function(){ hasChange = true },
			checkFormChange: checkFormChange
		}
	})())
...
(function(window, $){
	/**
	 * [HandyTable description]
	 * @deprecated
	 * @param {[type]} $element [description]
	 * @param {[type]} options  [description]
	 */
	var HandyTable = function($element, options){
		this.$element = $element;
		this.options = options;
	}
	HandyTable.prototype = {
		constructor: HandyTable,
		addRow: function(data, callback){
			if(data && data.length){
				for(var i = 0; i < data.length; i++){
					this.addRow(data[i], callback);
				}
				return;
			}
			data && (data.id = this.getRowsNum() + 1)
			var tplid = this.options.tplid,
				$row = $.tmpl(tplid, data);
			this.$element.append($row);
			callback && callback($row, this.$element);
		},
		removeRow: function(index, callback){
			var $row;
			// 如果传入数字，则假设按序号删除
			if(index && typeof index === "number"){
				$row = this.$element.find("tr").eq(index);
			// 否则当作直接传入jq节点处理
			}else{
				$row = index;
			}
		...
		reorderRows: function(){
			var $badges = this.$element.find("[data-toggle='badge']");
			for(var i = 0, len = $badges.length; i < len; i++){
				$badges.eq(i).html(i + 1);
			}	
		},
		getRowsNum: function(){
			return this.$element.find("[data-toggle='badge']").length;
		}
	}
	Ibos.HandyTable = HandyTable;
})(window, window.jQuery);
...
$.fn.waiting = function(content, mode, lock){
	...
	content = content || "";
	mode = mode || "normal";
	tpl = '<div><img src="<%= Ibos.app.getStaticUrl(path) %>" width="<%= size %>" height="<%= size %>" />' +
...
Ibos.OrderList.prototype.addItem = function(data, prepend) {
	data = $.extend({
		index: this.index
	}, data);
	this.index++;
...
Ibos.OrderList.prototype.removeItem = function(ids){
...
Ibos.OrderList.prototype.reorder = function(){

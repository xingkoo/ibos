...
(function(){
	var UPLOAD_TEXT = {
		uploading:              "等待上传",//"上传中...",
		queueLimitExceeded:     "已达到文件数上限",
		fileExceedsSizeLimit:   "文件太大",
		zeroByteFile:           "不能上传零字节文件",
		invalidFiletype:        "禁止上传该类型的文件",
		unknownError:           "未知错误",
		uploadComplete:         "上传完成",
		deleteAttach:           "删除附件"
		// HTTP_ERROR: "服务器没有正确响应",
		// MISSING_UPLOAD_URL: "未设置上传地址",
		// UPLOAD_FAILED: "上传失败",
		// IO_ERROR: "服务器写入错误",
		// SECURITY_ERROR: "安全性错误",
		// UPLOAD_LIMIT_EXCEEDED: "已达到文件数上限",
	...
	var imgQueueError = function(file, error, message){
		try{
			...
			switch (error) {
				case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
					Ui.tip(UPLOAD_TEXT.queueLimitExceeded, "warning");
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					Ui.tip(UPLOAD_TEXT.fileExceedsSizeLimit, "warning");
					break;
				case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
					Ui.tip(UPLOAD_TEXT.zeroByteFile, "warning");
					break;
				case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
					Ui.tip(UPLOAD_TEXT.invalidFiletype, "warning");
					break;
				default:
					if (file !== null) {
						Ui.tip(UPLOAD_TEXT.unknownError, "warning");
					}
					break;
			}
		} catch(e){
	...
	var getType = function(type){
		var types = ["7z", "ai", "bat", "bmp", "css", "doc", "exe", "fla", "fw", "gif", "html", "psd", "jpg", "js", "mp3", "pdf", "png", "ppt", "rar", "swf", "txt", "wma", "xls", "zip"];
		var typemap = {
			"pptx": "ppt",
			"docx": "doc",
			"xlsx": "xls"
		...
		return types.indexOf(type) !== -1 ? type : "unknown"
	}
	...
	Ibos.fileUpload = function(options){
		...
		return new SWFUpload($.extend(true, {}, _settings, options))
	}
	...
	var _attachFileQueueError = function(file, error, message){
		try{
			...
			switch (error) {
				case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
					progress.setStatus(U.lang("UPLOAD.QUEUE_LIMIT_EXCEEDED"));
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					progress.setStatus(U.lang("UPLOAD.FILE_EXCEEDS_SIZE_LIMIT"));
					break;
				case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
					progress.setStatus(U.lang("UPLOAD.ZERO_BYTE_FILE"));
					break;
				case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
					progress.setStatus(U.lang("UPLOAD.INVALID_FILETYPE"));
					break;
				default:
					if (file !== null) {
						progress.setStatus(U.lang("UPLOAD.UNKNOWN_ERROR"));
					}
					break;
			}
	...
	var _attachUploadSuccess = function(file, serverData, response){
		try {
			...
			type = getType(type);
			src = (data && data.imgurl) ? data.imgurl :  Ibos.app.getStaticUrl("/image/file/i_" + type + "_lt.png");
		...
		var _settings = {
			...
			custom_settings: {
				...
				template: '<div class="attl-item" data-node-type="attachItem">' +
					...
					'<i class="atti"><img width="32" height="32" src="<%=icon%>" alt="<%=name%>" title="<%=name%>" /></i>' +
	...
	var _imageFileQueueError = function(file, error, message){
		try{
			...
			switch (error) {
				case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
					Ui.tip("@UPLOAD.QUEUE_LIMIT_EXCEEDED", "danger");
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					Ui.tip("@UPLOAD.FILE_EXCEEDS_SIZE_LIMIT", "danger");
					break;
				case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
					Ui.tip("@UPLOAD.ZERO_BYTE_FILE", "danger");
					break;
				case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
					Ui.tip("@UPLOAD.INVALID_FILETYPE", "danger");
					break;
				default:
					if (file !== null) {
						Ui.tip("@UPLOAD.UNKNOWN_ERROR", "danger");
					}
					break;
			}
		} catch(e){
	...
	var _imageUploadError =function(file, error, message){
		try {
			...
			switch (error) {
				case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
					Ui.tip("Upload Error: " + message, "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
					Ui.tip("Configuration Error", "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
					Ui.tip("Upload Failed.", "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.IO_ERROR:
					Ui.tip("Server (IO) Error", "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
					Ui.tip("Security Error", "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
					Ui.tip("Upload limit exceeded.", "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
					Ui.tip("File not found.", "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
					Ui.tip("Failed Validation.  Upload skipped.", "danger");
					break;
				case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
					Ui.tip("Cancelled", "danger");
					progress.setCancelled();
					break;
				case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
					Ui.tip("Stopped", "danger");
					break;
				default:
					Ui.tip("Unhandled Error: " + message, "danger");
					break;
			}
		} catch (e) {
			this.debug(e);

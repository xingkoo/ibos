...
class HproseHttpClient
{
	...
	private $filter;
	static 	private $cookieManager = array();

	static public function hproseKeepCookieInSession()
	{
		$_SESSION["HPROSE_COOKIE_MANAGER"] = self::$cookieManager;
	}

	static public function keepSession()
	{
	...
	public function invoke($functionName, &$arguments = array(), $byRef = false, $resultMode = HproseResultMode::Normal)
	{
		...
		$stream->close();
		$opts = array(
			"http" => array("method" => "POST", "header" => $this->getCookie() . "Content-Length: " . strlen($request) . "\r\n" . ($this->keepAlive ? "Connection: keep-alive\r\nKeep-Alive: " . $this->keepAliveTimeout . "\r\n" : "Connection: close\r\n"), "content" => $request, "timeout" => $this->timeout / 1000)
			);

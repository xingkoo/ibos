...
{
	private $errorTable = array("E_ERROR\000\030" => NULL, "E_WARNING\000\030" => NULL, "E_PARSE\000\030" => NULL, "E_NOTICE\000\030" => NULL, "E_CORE_ERROR\000\030" => NULL, "E_CORE_WARNING\000\030" => NULL, "E_COMPILE_ERROR\000\030" => NULL, "E_COMPILE_WARNING\000\030" => NULL, "E_USER_ERROR\000\030" => NULL, "E_USER_WARNING\000\030" => NULL, "E_USER_NOTICE\000\030" => NULL, "E_STRICT\000\030" => NULL, "E_RECOVERABLE_ERROR\000\030" => NULL);
	private $functions;
	private $funcNames;
	private $resultModes;
	private $debug;
	...
	private function doInvoke()
	{
		do {
			...
			$functionArgs = array();
			$byref = false;
			$tag = $this->reader->checkTags(array(HproseTags::TagList, HproseTags::TagEnd, HproseTags::TagCall));

			if ($tag == HproseTags::TagList) {
				$this->reader->reset();
				$functionArgs = &$this->reader->readList(false);
				$tag = $this->reader->checkTags(array(HproseTags::TagTrue, HproseTags::TagEnd, HproseTags::TagCall));

				if ($tag == HproseTags::TagTrue) {
					$byref = true;
					$tag = $this->reader->checkTags(array(HproseTags::TagEnd, HproseTags::TagCall));
				}
			}
	...
	public function add()
	{
		...
		case 3:
			if (is_callable($args[0]) && is_null($args[1]) && is_string($args[2])) {
				...
				if (is_string($args[0]) && is_string($args[2])) {

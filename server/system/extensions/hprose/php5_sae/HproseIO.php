...
abstract class HproseAbstractStream
{
	...
	abstract public function write($string, $length);
}
...
{
	static 	private $classCache1 = array();
	static 	private $classCache2 = array();

	static public function register($class, $alias)
	{
		self::$classCache1[$alias] = $class;
		self::$classCache2[$class] = $alias;
	}

	static public function getClassAlias($class)
	{
	...
	static public function getClass($alias)
	{
...
class HproseReader
{
	...
	public function unserialize($tag = NULL)
	{
		...
		case HproseTags:
			$result = $this->readInteger(false);
			break;

		case HproseTags:
			$result = $this->readLong(false);
			break;

		case HproseTags:
			$result = $this->readDouble(false);
			break;
		...
		case HproseTags:
			$result = $this->readInfinity(false);
			break;

		case HproseTags:
			$result = &$this->readDate(false);
			break;

		case HproseTags:
			$result = &$this->readTime(false);
			break;

		case HproseTags:
			$result = $this->readBytes(false);
			break;

		case HproseTags:
			$result = $this->readUTF8Char(false);
			break;

		case HproseTags:
			$result = $this->readString(false);
			break;

		case HproseTags:
			$result = $this->readGuid(false);
			break;

		case HproseTags:
			$result = &$this->readList(false);
			break;

		case HproseTags:
			$result = &$this->readMap(false);
			break;
	...
	public function readObject($includeTag = true)
	{
		...
		$fields = $this->classref[(int) $this->stream->readuntil(HproseTags::TagOpenbrace)][2][1][0];
		$this->classref;
		$this->classref;
...
class HproseWriter
{
	...
	public function serialize(&$variable)
	{
		...
		case "string":
			if ($variable == "") {
				...
				else if (($ref = $this->ref_search($variable)) !== false) {
					$this->writeRef($ref);
				}
				else if (is_utf8($variable)) {
		...
		case "object":
			if (($ref = $this->ref_search($variable)) !== false) {
				...
				else if ($variable instanceof HproseTime) {
	...
	public function writeDouble($double)
	{
		...
		else if (is_infinite($double)) {
			$this->writeInfinity(0 < $double);
		}
		else {
	...
	private function writeClass(&$class)
	{
		...
		for ($i = 0; $i < $count; $i++) {
			...
			if ($field[0] == "\000") {
				$field = substr($field, strpos($field, "\000", 1) + 1);
			}
...
{
	static public function serialize($variable)
	{
	...
	static public function unserialize($variable_representation)
	{

...
class Chinese
{
	...
	public $convertbig5 = false;
	public $unicode_table = array();
	public $config = array("SourceLang" => "", "TargetLang" => "", "GBtoUnicode_table" => "gb-unicode.table", "BIG5toUnicode_table" => "big5-unicode.table", "GBtoBIG5_table" => "gb-big5.table");

	public function Chinese($SourceLang, $TargetLang, $ForceTable = false)
	{
	...
	public function _lang($LangCode)
	{
		...
		else if (substr($LangCode, 0, 3) == "BIG") {
			return "BIG5";
		}
		else if (substr($LangCode, 0, 3) == "UTF") {
			return "UTF-8";
		}
		else if (substr($LangCode, 0, 3) == "UNI") {
			return "UNICODE";
		}
	}
	...
	public function OpenTable()
	{
		...
		if (($this->config["SourceLang"] == "GBK") || ($this->config["TargetLang"] == "GBK")) {
			...
			if (($this->config["SourceLang"] == "BIG5") || ($this->config["TargetLang"] == "BIG5")) {
		...
		for ($i = 0; $i < strlen($tabletmp); $i += 4) {
			...
			else if ($this->config["SourceLang"] == "UTF-8") {
				$this->unicode_table[$tmp["value"]] = "0x" . dechex($tmp["key"]);
			}
			else if ($this->config["TargetLang"] == "UNICODE") {
	...
	public function GB2312toBIG5($c)
	{
	...
	public function Convert($SourceText)
	{
		if ($this->config["SourceLang"] == $this->config["TargetLang"]) {
			return $SourceText;
		}
		else if ($this->iconv_enabled) {
			if ($this->config["TargetLang"] != "UNICODE") {
				...
				while ($SourceText != "") {
					if (127 < ord(substr($SourceText, 0, 1))) {
			...
			while ($SourceText != "") {
				if (127 < ord(substr($SourceText, 0, 1))) {
					if ($this->config["SourceLang"] == "GBK") {
						$utf .= "&#x" . $this->unicode_table[hexdec(bin2hex(substr($SourceText, 0, 2))) - 32896] . ";";
					}
					else if ($this->config["SourceLang"] == "BIG5") {
			...
			if ($this->config["SourceLang"] == "UTF-8") {
				...
				while ($i < $len) {
					...
					case 13:
						...
						else if ($this->config["TargetLang"] == "BIG5") {
				...
				while ($SourceText != "") {
					if (127 < ord(substr($SourceText, 0, 1))) {
						...
						else if ($this->config["SourceLang"] == "GBK") {
							$utf8 = $this->CHSUtoUTF8(hexdec($this->unicode_table[hexdec(bin2hex(substr($SourceText, 0, 2))) - 32896]));
						}
	...
	public function Utf8_Unicode($char)
	{
		switch (strlen($char)) {
		case 1:
			return ord($char);

...
<script>
	Ibos.app.s({
		calSettings : {
			// 日程开始时间
			minTime: Ibos.date.numberToTime(<?php echo CalendarUtil::getSetupStartTime( Ibos::app()->user->uid ); ?>),
			// 日程结束时间
			maxTime: Ibos.date.numberToTime(<?php echo CalendarUtil::getSetupEndTime( Ibos::app()->user->uid ); ?>),
			hiddenDays: [<?php echo CalendarUtil::getSetupHiddenDays( Ibos::app()->user->uid ); ?>],
...
<script type="text/javascript">
	$(function(){
		...
		// 保证至少有一个下属
		setTimeout(function(){
			Ibos.app.guide("cal_sch_sub", function(){
				$(".mng-item.sub").eq(0).addClass("introing");
				Ibos.intro([
					{ 
						element: ".mng-item.sub > div.pull-right",
						intro: U.lang("INTRO.CAL_SCH_SUB.VIEW_UNDERLINGS")
					}
				], function(){
					$(".mng-item.sub").eq(0).removeClass("introing");
					Ibos.app.finishGuide('cal_sch_sub');
				});
			})
		}, 1000)
	});

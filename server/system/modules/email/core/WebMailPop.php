...
class WebMailPop extends WebMailBase
{
    public function connect($host, $user, $password, $ssl = false, $port = "", $authMethod = "plain")
    {
        ...
        if ($conn->fp) {
            ...
            if (($authMethod == "auth") && $authMethods["cram-md5"]) {
                $result = $this->cramMD5($conn, $user, $password);
                $conn->message .= "CRAM_MD5: {$result}\n";
            }

            if (!$result && $tryApop && $timestamp && ($authMethod != "plain")) {
    ...
    public function fetchHeader(&$conn, $mailbox, $id)
    {
        ...
        if ($line[0] == "+") {
            ...
            for ($i = 0; $i < $numlines; $i++) {
                ...
                } elseif (strcasecmp($field, "from") == 0) {
                    $result->from = str_replace("\n", " ", $string);
                } elseif (strcasecmp($field, "to") == 0) {
                    $result->to = $string;
                } elseif (strcasecmp($field, "subject") == 0) {
                    $result->subject = str_replace("\n", "", $string);
                } elseif (strcasecmp($field, "reply-to") == 0) {
                    $result->replyto = $string;
                } elseif (strcasecmp($field, "cc") == 0) {
                    $result->cc = str_replace("\n", " ", $string);
                } elseif (strcasecmp($field, "Content-Transfer-Encoding") == 0) {
                    $result->encoding = $string;
                } elseif (strcasecmp($field, "message-id") == 0) {
    ...
    {
        if (0 < strpos($messageSet, ":")) {
            $a = explode(":", $messageSet);
            $from_i = (int) $a[0];
            $to_i = (int) $a[1];
        } elseif (0 < strpos($messageSet, ",")) {
            $a = explode(",", $messageSet);
            $n = count($a);
            $from_i = (int) $a[0];
            $to_i = (int) $a[$n - 1];
        ...
        for ($id = $from_i; $fp; $id++) {
            ...
            if ($line[0] == "+") {
                ...
                $result[$id]->cc = (isset($header_fields["reply-to"]) ? $header_fields["cc"] : "");
    ...
    protected function fetchBodyPart(&$conn, $boundary, &$last_line, $the_part, &$part, $action, $bytes_total, &$bytes_read)
    {
        ...
        if (strcasecmp($header["content-type"]["major"], "multipart") == 0) {
            ...
            do {
                ...
                $end = (0 < (strlen($last_line) - strlen($boundary))) || (chop($last_line) == ".");
        ...
        } elseif (strcasecmp($header["content-type"]["major"], "message") == 0) {
            do {
                $line = $this->readLine($conn);
    ...
    protected function readNParse($conn, $boundary, &$last_line)
    {
        ...
        if (strcasecmp($header["content-type"]["major"], "multipart") == 0) {
            ...
            if (!empty($header["content-disposition"]["data"])) {
                ...
                if (is_array($param_a) && (0 < count($param_a))) {
                    $str .= "(" . implode(" ", $param_a) . ")";
                } else {
                    $str .= "NIL";
                }
            ...
            if (is_array($header["content-type"]["parameters"]) && (0 < count($header["content-type"]["parameters"]))) {

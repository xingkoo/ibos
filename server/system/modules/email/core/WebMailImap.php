...
class WebMailImap extends WebMailBase
{
    ...
    public function fetchHeaderIndex(&$conn, $mailbox, $messageSet, $indexField)
    {
        ...
        $fields_a = array("DATE" => 6, "FROM" => 1, "TO" => 1, "SUBJECT" => 1, "UID" => 2, "SIZE" => 2, "SEEN" => 3, "RECENT" => 4, "DELETED" => 5);
                ...
                if (($line[0] == "*") && ($a[2] == "FETCH") && ($line[strlen($line) - 1] != ")")) {
                    ...
                    while ($line[strlen($line) - 1] != ")") {
                        ...
                        if ($line[0] != ")") {
                            ...
                            if ((32 < ord($line[0])) || (strlen($line[0]) == 0)) {
            ...
            do {
                ...
                if (($line[0] == "*") && ($a[2] == "FETCH")) {
                    ...
                    if ($mode == 2) {
                        ...
                        $result[$id] = (0 < strpos($haystack, $indexField) ? "F" : "N");
                    }
                }
                ...
                if ($open_pos && $close_pos) {
                    ...
                    if (8 <= $parts_count) {
                        ...
                        if (is_array($flags_a)) {
                            ...
                            while (list($key, $val) = each($flags_a)) {
                                if (strcasecmp($val, "Seen") == 0) {
                                    $result[$id]->seen = true;
                                } elseif (strcasecmp($val, "Deleted") == 0) {
                                    $result[$id]->deleted = true;
                                } elseif (strcasecmp($val, "Recent") == 0) {
                                    $result[$id]->recent = true;
                                } elseif (strcasecmp($val, "Answered") == 0) {
                        ...
                        if (!empty($time_str)) {
                            ...
                            $time_zone_str = substr($time_str, -5);
                            $time_str = substr($time_str, 1, -6);
                            $time_zone = (int) substr($time_zone_str, 1, 2);

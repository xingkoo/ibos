<div class="vote vote-pic well well-lightblue">
		...
		<div class="media-body">
            ...
            <div class="tcm">
				<?php if ($voteStatus == 1) : ?>
					...
					<?php elseif (is_array($voteData["vote"]["remainTime"])) : ?>
						<?php echo Ibos::lang("Distance vote end time", "vote.default");?>
						<?php echo $voteData["vote"]["remainTime"]["day"];?>
						<?php echo Ibos::lang("Day", "date");?>
						<?php echo $voteData["vote"]["remainTime"]["hour"];?>
						<?php echo Ibos::lang("Hour", "date");?>
						<?php echo $voteData["vote"]["remainTime"]["minute"];?>
						<?php echo Ibos::lang("Min", "date");?>
						<?php echo $voteData["vote"]["remainTime"]["second"];?>
						<?php echo Ibos::lang("Sec", "date");?>
					<?php endif; ?>
				<?php elseif ($voteStatus == 0) : ?>
					<?php echo Ibos::lang("Closed", "vote.default");?>
				<?php endif; ?>
				| <?php if ($voteData["vote"]["ismulti"] == 0) ?><?php echo Ibos::lang("Single select", "vote.default");?><?php else : ?><?php echo Ibos::lang("Multi select", "vote.default") . " | " . Ibos::lang("Max select number", "vote.default") . $voteData["vote"]["maxselectnum"] . Ibos::lang("Item", "vote.default");?><?php endif; ?>
			</div>
		</div>
	</div>
	...
	<?php if ((($voteStatus == 1) || ($voteStatus == 0)) && ($userHasVote == true)) : ?>
		<div class="vote-body">
   		...
   		<p><?php echo Ibos::lang("Thank you for participating", "vote.default");?>
	</p>
	<!--判断投票结果查看权限  如果所有人可见：显示投票结果，显示投票按钮； 如果为投票后可见，不显示投票结果，投票后显示-->
	<?php else : ?>
		<?php if (($voteStatus == 1) && ($userHasVote == false)) : ?>
        <!--如果所有人可见：显示投票结果-->
		...
		<?php elseif ($voteData["vote"]["isvisible"] == 1) : ?>
	  	<div class="vote-body">
			<ul class="vote-pic-list clearfix" id="vote">
				<?php foreach ($voteData["voteItemList"] as $voteItem ) : ?>
					<li data-id="<?php echo $voteItem["itemid"];?>">
		...
		var $vote = $("#vote"),
		 ...
		 $('#vote_submit').on('click',function(){
             var $elem = $(this),
				...
				if(!voteItemids){
					// @Debug: PHP语言包出错
                    $.jGrowl('<?php echo Ibos::lang("Min select num description");?>');
					return false;
				}
				var url="<?php echo $this->createUrl("default/edit", array("op" => "clickVote"));?>";
				$.post(url, {

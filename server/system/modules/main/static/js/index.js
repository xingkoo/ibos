 ...
 * IBOS
 * @author		inaki
 * @version		$Id: index.js 3538 2014-06-03 08:06:06Z gzzcs $
 * @modified	2013-09-02
 */
	...
	return {
		...
		add: function(name, options, callback){
			if(nameIndexOf(name, data) === "-1"){
				$.error("(addModule): " +  U.lang("IN.MODULE_NOT_FOUND", { modname: name}));
			}
		...
		remove: function(name, callback){
			if(nameIndexOf(name, data) === "-1"){
				$.error("(removeModule): " + U.lang("IN.MODULE_NOT_FOUND", { modname: name}));
			}
			//移除模块时，从本地缓存中删除
...
var In = {
	startIntro: function(){
		...
		$(function(){
			setTimeout(function(){
				Ibos.app.guide("index", function() {
					var guideData = [];
					if($("#module_panel .mbox").length){
						guideData.push({ 
							element: "#module_panel .mbox", 
							intro: U.lang("INTRO.INDEX.MOD_DRAG"),
							position: "right"
						});
					}
					guideData.push({
						...
						// element: "#in_operate",
						intro: U.lang("INTRO.INDEX.MOD_SETTING"),
						position: "left"
					})
					// manager.show();
					Ibos.intro(guideData, function(){
						// manager.hide();
						Ibos.app.finishGuide('index');
					});
				})
			}, 1000);
		});

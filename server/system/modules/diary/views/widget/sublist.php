						...
						<?php foreach ( $dept['user'] as $user ): ?>
							<li>
								<div class="mng-item mng-item-user">
									...
									<a href="<?php echo $this->getController()->createUrl( $userRoute, array( 'uid' => $user['uid'] ) ); ?>" <?php if ( EnvUtil::getRequest( 'uid' ) == $user['uid'] ): ?>style="color:#3497DB;"<?php endif; ?>>
										<img src="avatar.php?uid=<?php echo $user['uid']; ?>&size=middle&engine=<?php echo ENGINE; ?>" alt="<?php echo $user['realname']; ?>">
										<?php echo $user['realname']; ?>
									</a>
									<!-- if 未关注 -->
									<?php if ( DiaryUtil::getIsAttention( $user['uid'] ) ): ?>

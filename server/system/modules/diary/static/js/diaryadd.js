...
$(function(){
	        ...
	        "addFromCalendar": function(param, elem) {
	            $.get(Ibos.app.url('diary/default/add', {'op': 'planFromSchedule'}), {
	                ...
	                if (res.length === 0) { //如果没有日程，给个提示
	                	Ui.tip(U.lang("DA.YOU_DO_NOT_SCHEDULE_TODAY"), "warning");
	                    return false;
	                }
	                //否则有日程，添加到计划外
	...
	// 新手引导
	setTimeout(function(){
	    Ibos.app.guide("dia_def_add", function() {
	        var guideData = [];

	        if(document.getElementById("from_calendar_btn")){
	            guideData.push({
	                element: "#from_calendar_btn",
	                intro: U.lang("INTRO.DIA_DEF_ADD.ADD_FROM_SCHE")
	            });
	        }
	        ...
	        if(document.getElementById("share_intro")) {
	            guideData.push({
	                element: "#share_intro",
	                intro: U.lang("INTRO.DIA_DEF_ADD.ADD_SHARE"),
	                position: "top"
	            });
	        }

	        Ibos.intro(guideData, function(){
	            Ibos.app.finishGuide('dia_def_add');
	        });
	    })
	}, 1000);
});
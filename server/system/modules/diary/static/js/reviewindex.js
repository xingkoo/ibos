...
$(function(){
	...
	$('#da_date_btn').datepicker().on("changeDate", function(evt) {
		...
		window.location.href = Ibos.app.url('diary/review/index', { date: date});
	});

	// 新手引导
	setTimeout(function(){
		Ibos.app.guide("dia_rev_index", function(){
			var guideData = [];

			if($("#mng_list .mng-item-user").length) {
				guideData.push({
					element: "#mng_list .mng-item-user", 
					intro: U.lang("INTRO.DIA_REV_INDEX.SHOW_UNDERLINGS")
				});
			};
			...
			if($(".o-da-bell").length) {
				guideData.push({
					element: ".o-da-bell", 
					intro: U.lang("INTRO.DIA_REV_INDEX.REMIND_UNDERLINGS")
				});
			}
			...
			if($(".da-list-item").length){
				guideData.push({
					...
					intro: U.lang("INTRO.DIA_REV_INDEX.MARK_UNDERLINGS")
				});
			}
			if(guideData.length){
				Ibos.intro(guideData, function(){
					Ibos.app.finishGuide('dia_rev_index');
				});
			}
		})
	}, 1000)

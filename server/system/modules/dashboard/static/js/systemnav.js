var navigation = {
	...
	resetMainNavLi : function($elem){
		...
		for(var index = 0; index < mainLiLength; index++){
			var $mianLi = $elem.eq(index),
				...
				addChildBtnLength = $mianLi.find(".w30").eq(0).find(".o-plus").length;
			...
			//当主导航缺少添加子导航按钮时，添加按钮
			if(!addButtonLength){
				$mianLi.find(".w30").append(addChindLiHtml);
			}else{
				// $mianLi.find(".w30").eq(0).find(".o-trash").remove();
				if(!addChildBtnLength){
					$mianLi.find(".w30").eq(0).find(".add-child-btn").append(addButtonHtml);
				}
			}
	...
	moveMainLiToChildLi : function($main, $elem, $target){
		...
		if(nodetpye != "nav_main_list"){
			...
			if(chilsNavliLength){
				...
				Ui.tip(L.DB.REMOVE_CHILD_NAVIGATION, "danger");
			}
		}
	},
	/**
	...
	collapseChildNav : function($elem, state){
		if(state === "start"){
			$elem.find(".nav-child-list").hide();
		}else{
			$elem.find(".nav-child-list").show();
		}
		// $elem.find(".nav-child-list").toggle(state === "start");
	},
	...
	openPostWindow: function(content, url) {
...
var op = {
	_fetchData: function($elem){
		var $checkboxes = $elem.find(":checkbox"),
			$select = $elem.find(".w14").children().find("option:selected"),
			index = $select.val(),
			type = $elem.find(".w14").attr("data-type"),
...
$(function(){
	...
	$("#nav_main_list").on("sortstart", function(e, ui){
		var $mainLi = ui.item,
			$mainItemLi = ui.item.find(".main-nav-item"),
			state = "start";
	...
	$("#nav_main_list").on("sortstop", function(e, ui){
		var $mainNavLi = $("#nav_main_list>li"),
			$currentLi = ui.item,
			$currentMainLi = ui.item.find(".main-nav-item"),
			state = "stop";
	...
	$("#sys_nav_form").delegate('.type-select', 'change', function() {
		var index = $(this).find('option:selected').val(),
			$navType= $(this).parent(),
			$linkList = $(this).parent().siblings(".w40").children(),
	...
	Ibos.evt.add({
		"isUse": function(param, elem){
			var checkboxVal = $(elem).prop("checked"),
				$childCheckbox = $(elem).closest('li').find(".nav-child-list").find(".isuse").find(":checkbox");

			if(!checkboxVal){
				$childCheckbox.prop("checked", checkboxVal).label("refresh");
			}
		},
		"previewPage": function(param, elem){
			var pageid = $(elem).find("[data-pageid]").attr("data-pageid"),
				name = $(elem).find("[data-type='name']").val();
		}
	});
});
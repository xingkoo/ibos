						...
						<?php foreach ( $list as $module => $data ): ?>
							...
							<li class="main-list-item" id="remind_<?php echo $module; ?>">
								<div class="avatar-box pull-left posr" >
									<img class="mbm" width="64" src="<?php echo Yii::app()->assetManager->getAssetsUrl($module).'/image/icon.png';?>" alt="">
								...
								<div class="main-list-item-body">
												...
												<div>
													<ul class="clist">
														<?php foreach ( $data['newlist'] as $k=>$newMsg ): ?>
																		<?php if ( $k == 0 ) {
																			$time = ConvertUtil::formatDate( $newMsg['ctime'], 'u' );
																		} ?>
															<li>
																<?php if( empty( $newMsg['url'] ) ): ?>
																	<?php echo $newMsg['title']; ?>
																<?php else: ?>
																	<a href="<?php echo $newMsg['url']; ?>"><?php echo $newMsg['title']; ?></a>
																<?php endif; ?>
															</li>
														<?php endforeach; ?>
													</ul>
									...
									<?php else: ?>
										<?php $time =  ConvertUtil::formatDate($data['latest']['ctime'],'u'); ?>
										<div class="mb">
											<p>

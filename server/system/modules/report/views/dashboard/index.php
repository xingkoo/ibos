<link rel="stylesheet" href="<?php echo Yii::app()->assetManager->getAssetsUrl("report");?>/css/dbreport.css?<?php echo VERHASH;?>">
<div class="ct">
	<div class="clearfix">
			...
			<div class="ctb">
				...
				<div class="ctbw">
					<div class="control-group">
						...
						<div class="controls">
							<input type="checkbox" id="stamp_switch" name="stampenable" value='1' id="" data-toggle="switch" class="visi-hidden" <?php if ($config["stampenable"]) echo "checked";?> >
						</div>
					</div>
				</div>
				<!-- 图章设置 -->
						...
						<div class="control-group">
							<div class="controls">
								<select name="pointsystem" id="point_sys" class="span6">
									<option value="10" <?php if ($config["pointsystem"] == 10) echo "selected";?> ><?php echo $lang["Ten point scale"];?></option>
									<option value="5" <?php if ($config["pointsystem"] == 5) echo "selected";?> ><?php echo $lang["Five point scale"];?></option>
									<option value="3" <?php if ($config["pointsystem"] == 3) echo "selected";?> ><?php echo $lang["Three point scale"];?></option>
								</select>
							</div>
						</div>
					</div>
								...
								<table id="stamp_slot_table" class="table table-bordered table-striped ps-table pull-left" style="width: 300px;">
									<thead>
										<tr>
											...
											<th>
												<label class="checkbox pull-right" title=""><input type="checkbox" name="autoreview" id="auto_apprise" value="1"><?php echo $lang["Auto comment"];?></label>
											</th>
										</tr>
									</thead>
									<tbody>
										<?php if (0 < count($stamps)) : ?>
											<?php foreach ($stamps as $score => $stampId ) : ?>
												<!--大于上面所选的积分制的条数就隐藏-->
												<?php if (($config["pointsystem"] < $score) || ($stampId == 0)) : ?>
													<tr <?php if ($config["pointsystem"] < $score) echo 'style="display: none;"';?> >
														...
														<td>
															<div class="stamp-slot" data-node-type="stampSlot">
																<div class="stamp-item ui-draggable" data-node-type="stampItem" data-stamp-id="<?php echo $stampId;?>">
																	<img src="<?php echo FileUtil::fileName("data/stamp/" . Stamp::model()->fetchIconById($stampId));?>" alt="" width="60" height="24">
																</div>
															</div>
															<input type="hidden" name="stampdetails[<?php echo $score;?>]" value="<?php echo $stampId;?>">
														</td>
														<td>
															<label class="radio pull-right" style="display: none;"><input type="radio" name="apprise" value="<?php echo $score;?>" <?php if ($config["autoreviewstamp"] == $stampId) echo "checked";?> ></label>
														</td>
													</tr>
												<?php endif; ?>
											<?php endforeach; ?>
												...
												<div id="stamp_item_box" class="stamp-item-box">
													<?php foreach ($diffStampIds as $key => $diffStampId ) : ?>
														<div class="stamp-item" data-node-type="stampItem" data-stamp-id="<?php echo $diffStampId;?>">
															<img src="<?php echo FileUtil::fileName("data/stamp/" . Stamp::model()->fetchIconById($diffStampId));?>" alt=""  width="60" height="24">
														</div>    
													<?php endforeach; ?>
												</div>
											</td>
...
<script src="<?php echo Yii::app()->assetManager->getAssetsUrl("report");?>/js/dbreport.js?<?php echo VERHASH;?>"></script>
<script>

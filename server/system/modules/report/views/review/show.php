					...
					<div class="usi-terse">
						<a href="" class="avatar-box">
							<span class="avatar-circle">
								<img class="mbm" src="avatar.php?uid=<?php echo $report["uid"];?>&size=middle&engine=<?php echo ENGINE;?>" alt="">
							</span>
						</a>
			...
			<div class="page-list">
				<div class="page-list-header">
					<div class="btn-group pull-right">
						<a 
						<?php if (!empty($preAndNextRep["preRep"])) : ?>
						...
						<a 
						<?php if (!empty($preAndNextRep["nextRep"])) : ?>
				...
				<div class="page-list-mainer posr">
						...
						<tbody>
							<div class="rp-stamp">
								<span id="preview_stamp">
									<?php if (0 < $report["stamp"]) : ?>
										<img id="stamp_<?php echo $report["repid"];?>" src="<?php echo FileUtil::fileName(Stamp::STAMP_PATH . $stampUrl);?>" width="150px" height="90px" />
									<?php endif; ?>
								</span>
							</div>
							<tr>
									...
									<tr>
										...
										<td>
											<div class="fill">
												...
												<span class="rp-detail-num"><?php echo $k1 + 1;?>.</span> <?php echo $orgPlan["content"];?>												<div class="rp-exec-status">
													<?php echo $lang["Implementation"];?>：<?php echo $orgPlan["exedetail"];?>												</div>
											</div>
										</td>
									</tr>
								<?php endforeach; ?>
							...
							<?php if (!empty($nextPlanList)) : ?>
								...
								<?php foreach ($nextPlanList as $k4 => $nextPlan ) : ?>
									<tr>
										...
										<td>
											<div class="fill">
												<span class="rp-detail-num"><?php echo $k4 + 1;?>.</span> <?php echo $nextPlan["content"];?>											</div>
										</td>
									</tr>
								<?php endforeach; ?>
							<?php endif; ?>
					...
					<div class="cti bdbs">
						...
						<div id="report_comment" data-url="<?php echo Yii::app()->urlManager->createUrl("message/comment/getcomment");?>">
							<?php 
								$this->widget("IWReportComment", array(
									"module"     => "report",
									"table"      => "report",
									"attributes" => array("rowid" => $report["repid"], "moduleuid" => Ibos::app()->user->uid, "touid" => $report["uid"], "module_rowid" => $report["repid"], "module_table" => "report", "api" => "reviewSubordinate", "allowComment" => 1, "showStamp" => $this->issetStamp())
								));
							?>
                        </div>
					</div>
						...
						<div>
							...
							<?php foreach ($readers as $reader ) : ?>
								<a href="<?php echo Ibos::app()->createUrl("user/home/index", array("uid" => $reader["uid"]));?>">
									<img src="avatar.php?uid=<?php echo $reader["uid"];?>&size=small&engine=<?php echo ENGINE;?>" title="<?php echo $reader["realname"];?>" class="img-rounded"/>
								</a>
							<?php endforeach; ?>
							</div>
						</div>
...
<script>
	Ibos.app.setPageParam({
		...
		stampPath: '<?php echo FileUtil::fileName(Stamp::STAMP_PATH);?>',
	})
</script>

...
<table class="rp-detail-table">
	<tbody>
			...
			<td colspan="3">
				<div class="rp-detail-header curp clearfix" data-action="hideReportDetail">
					<div class="mini-date fill-ss pull-left">
						<h4>
							<?php if ($fromController == "review") : ?>
								<?php echo User::model()->fetchRealnameByUid($report["uid"]);?>
							<?php endif; ?>
		...
		<!--原计划-->
				...
				<tr>
					...
					<td>
						<div class="fill">
							...
							<?php $lang["Implementation"];?>：<?php echo $orgPlan["exedetail"];?>
							</div>
							<?php endif; ?>
						</div>
					</td>
			...
			<?php endif; ?>
		<!--计划外-->
			<?php if (0 < count($outSidePlanList)) : ?>
					...
					<td>
						<div class="fill">
							<div class="bamboo-pgb pull-right">
								...
								<span data-toggle="bamboo-pgb" data-value="<?php echo $outSidePlan["process"];?>"></span>
			...
			<tr>
				...
				<td>
					<div class="fill" style="min-height: 180px; width: 620px">
						<p class="summary"><?php echo $report["content"];?></p>
					</div>
				</td>
			</tr>
		<!--附件-->
...
<div class="cti bdbs">
	...
	<div id="load_comment_<?php echo $report["repid"];?>" data-id="<?php echo $report["repid"];?>" data-url="<?php echo Yii::app()->urlManager->createUrl("message/comment/getcomment");?>" >
		<?php 
			$this->widget("IWReportComment", array(
				"module"     => "report",
				"table"      => "report",
				"attributes" => array("rowid" => $report["repid"], "moduleuid" => Ibos::app()->user->uid, "touid" => $report["uid"], "module_rowid" => $report["repid"], "module_table" => "report", "api" => "reviewSubordinate", "allowComment" => $allowComment, "showStamp" => ($fromController == "review") && $this->issetStamp())
			));
		?>
	</div>
</div>

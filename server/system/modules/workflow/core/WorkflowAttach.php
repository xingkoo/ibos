...
class WorkflowAttach extends ICAttach
{
    public function upload()
    {
        $uid = intval(EnvUtil::getRequest("uid"));
        $flowprocess = EnvUtil::getRequest("flowprocess");
        $this->upload->save();
        $attach = $this->upload->getAttach();
        $attachment = $attach["type"] . "/" . $attach["attachment"];
        $data = array("dateline" => TIMESTAMP, "filename" => $attach["name"], "filesize" => $attach["size"], "attachment" => $attachment, "isimage" => $attach["isimage"], "description" => $flowprocess, "uid" => $uid);

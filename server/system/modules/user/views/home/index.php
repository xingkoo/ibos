				...
				<div class="mbox-body pc-hb">
						...
						<div>
							...
							<div class="mbs">
								<div class="dib">
									...
									<span>
									<?php 
										if (90 < $user["upgrade_percent"]) {
											echo $lang["Credit Encourage 1"];
										} else {
											echo $lang["Credit Encourage 2"];
										}
									?>
									</span>
								</div>
							...
							<div class="progress" id="exp_info">
								<div class="progress-bar <?php if (90 < $user["upgrade_percent"]) : ?>progress-bar-danger<?php else : ?>progress-bar-success<?php endif; ?>" style="width: <?php echo $user["upgrade_percent"];?>%;"></div>
							</div>
							<div>
								<div class="dib">
					...
					<div class="mb">
						<table class="profile-info-table">
							<tbody>
								<tr>
									<?php foreach ($extcredits as $index => $credits ) : ?>
								...
								<ul>
									<li class="clearfix mbs">
										<?php if ($user["validationemail"] == 1) : ?>
											<i class="o-small-mail-bind"></i>
											<?php echo $lang["Email address"];?>    
											：
											<?php echo $user["email"];?>    
											<span class="tcm">
												<?php echo $lang["Already bind"];?></span>
											<a href="javascript:;" data-act="bind" data-type='email' class="btn btn-small pull-right">
												<?php echo $lang["Modify"];?>
											</a>
										<?php else : ?>    
									...
									<li class="clearfix">
										<?php if ($user["validationmobile"] == 1) : ?>    
											...
											<span class="tcm">
												<?php echo $lang["Already bind"];?></span>
											<a href="javascript:;" data-act="bind" data-type='mobile' class="btn btn-small pull-right">
				...
				<div class="mbox mbox-stable">
					...
					<div class="mbox-body">
						<table class="table table-striped mbz">
							<tbody>
								<?php $terminal = array("web" => $lang["Web login"], "app" => $lang["App login"]);?>
								<?php foreach ($history as $log ) : ?>    
									<?php $row = json_decode($log["message"], true);?>    
									<tr>
										<td>
											<?php echo ConvertUtil::formatDate($log["logtime"], "u");?></td>
										<td>
											<?php echo StringUtil::cutStr(!empty($row["address"]) ? $row["address"] : ConvertUtil::convertIp($row["ip"]), 7);?></td>
										<td>
											<?php echo $row["ip"];?></td>
										<td>
						...
						<div class="mbox-layer">
							<div class="xac fill-nn">
								<a href="<?php echo $this->createUrl("home/personal", array("op" => "history"));?>" class="link-more">
...
<script src='<?php echo STATICURL;?>/js/lib/echarts/asset/js/esl/esl.js?<?php echo VERHASH;?>'></script>
...
<?php if (!empty($contacts)) : ?>
		...
		<?php foreach ($contacts as $index => $contact ) : ?>
				{category: <?php echo $index <= 2 ? $index : "2"; ?>, name: '<?php echo $contact["realname"];?>', value: <?php echo rand(1, 40);?>},
		<?php endforeach; ?>
			],
			links: [
		<?php foreach ($contacts as $index => $contact ) : ?>
				{source: <?php echo $index + 1;?>, target: 0, weight: 1},
		<?php endforeach; ?>
			]
		}];
		var myChart = contactForce(contacts, {series: seriesData});
<?php else : ?>
...
<?php if ($this->getIsMe()) : ?>
	...
	$securityProgress.on({
		...
		if (data.value < 60) {
			...
			style = "xcr";
			desc = '<?php echo $lang["Security level 1"];?>';

		} else if (data.value < 100) {
			progress.setStyle("warning");
			style = "xco";
			desc = '<?php echo $lang["Security level 2"];?>';
		} else if (data.value === 100) {
			progress.setStyle("success");
			style = "xcgn";
			desc = '<?php echo $lang["Security level 3"];?>';
		}
	...
	progress.setValue(<?php echo $securityRating;?>);
	$recheck.on('click', function() {
		var $that = $(this), url = '<?php echo $this->createUrl("home/checkSecurityRating", array("uid" => $this->getUid()));?>';
		
		$that.button('loading');
		$.get(url, function(data) {
			if (data.IsSuccess) {
				progress.setValue(data.rating)
			} else {
				Ui.tip('<?php echo Ibos::lang("Operation failure", "message");?>', 'danger');
			}
				$that.button('reset');
		}, 'json');
	});

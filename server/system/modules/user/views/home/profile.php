			...
			<div>
				<form id="profile_form" action="<?php echo $this->createUrl("home/personal");?>" method="post" class="form-horizontal form-narrow">
					...
					<div class="control-group">
						...
						<div class="controls">
							<div class="datepicker span5" id="birthday">
								...
								<input type="text" name="birthday" class="datepicker-input" value="<?php if (0 < $user["birthday"]) echo date("Y-m-d", $user["birthday"]);?>">
							</div>
						</div>
					</div>
					<div class="control-group">
					...
					<div class="control-group">
						...
						<div class="controls">
							<input type="text" value="<?php echo $user["mobile"];?>" id="mobile" name="mobile" class="span7"<?php if ($user["validationmobile"] == 1) echo " disabled ";?> >
						</div>
					</div>
					<div class="control-group">
						<label class="control-label"><?php echo $lang["Email"];?></label>
						<div class="controls">
							<input type="text" value="<?php echo $user["email"];?>" id="email" name="email" class="span7"<?php if ($user["validationemail"] == 1) echo " disabled";?> >
						</div>
					</div>
					<div class="control-group">
		...
		<div class="other-info pull-left">
			...
			<form action="" method="post" class="form-horizontal form-narrow">
				...
				<div class="control-group">
					...
					<div class="controls">
						<div class="controls-content">
							<span class="tcm"><?php if (0 < $user["birthday"]) echo date("Y-m-d", $user["birthday"]);?></span>
						</div>
					</div>
				</div>
				<div class="control-group">
...
<script>
	...
	/*$.formValidator.initConfig({ formID: "profile_form", errorFocus:true });
	 ...
	 .ajaxValidator({	//验证手机是否已被注册
	 type : 'GET',
	 datatype : "json",
	 async : true,
	 url : "<?php echo Yii::app()->createUrl("organization/user/IsRegistered", array("uid" => $this->getUid()));?>",
	 success : function(res){
	 var data = $.parseJSON(res);
	 //数据是否可用？可用则返回true，否则返回false
	 var isAvailable = data.IsSuccess ? true : false;
	 return isAvailable;
	 },
	 buttons: $(".btn btn-large btn-primary btn-great"),
	 onError : L.MOBILE + L.EXIST,
	 });

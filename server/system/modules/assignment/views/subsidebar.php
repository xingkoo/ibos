										...
										<?php foreach ($dept["user"] as $user ) : ?>
											<li>
												<div class="mng-item sub">
													<span class="o-caret g-sub" data-action="toggleSubUnderlingsList" data-param='{"uid":"<?php echo $user["uid"];?>"}'>
														<?php if ($user["hasSub"]) : ?>
															<i class="caret"></i>
														<?php endif; ?>
													</span>
													<a href="<?php echo $this->createUrl("unfinished/subList", array("uid" => $user["uid"]));?>" <?php if (EnvUtil::getRequest("uid") == $user["uid"]) : ?>style="color:#3497DB;"<?php endif; ?> >
														<img src="avatar.php?uid=<?php echo $user["uid"];?>&size=middle&engine=<?php echo ENGINE;?>" alt=""><?php echo $user["realname"];?>
													</a>
												</div>
												<!--下属资料,ajax调用生成-->
											</li>

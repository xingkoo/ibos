...
$(function() {
	Ibos.evt.add({
		...
		"removeWbs": function(param, elem) {
			...
			var tips = param.op == 'delFeed' ? U.lang("WB.REMOVE_SELECT_FEEDS_CONFIRM") : U.lang("WB.DELETE_SELECT_FEEDS_CONFIRM");
			WbDash.removeAccess(wbIds, Ibos.app.url('weibo/dashboard/manage'), param, tips);
		},
		// 删除一条微博
		"removeWb": function(param, elem) {
			WbDash.removeAccess(param.id, Ibos.app.url('weibo/dashboard/manage'), param, U.lang("WB.REMOVE_SELECT_FEEDS_CONFIRM"));
		},
		// 恢复一条微博
		"recoverWb": function(param, elem) {
			WbDash.removeAccess(param.id, Ibos.app.url('weibo/dashboard/manage'), param, U.lang("WB.RECOVER_SELECT_FEED_CONFIRM"));
		},
		// 删除选中评论
		"removeCms": function(param, elem) {
			var cmIds = U.getCheckedValue("comment");
			var tips = param.op == 'delComment' ? U.lang("WB.REMOVE_SELECT_COMMENTS_CONFIRM") : U.lang("WB.DELETE_SELECT_COMMENTS_CONFIRM");
			WbDash.removeAccess(cmIds, Ibos.app.url('weibo/dashboard/comment'), param, tips);
		},
		// 删除一条评论
		"removeCm": function(param, elem) {
			WbDash.removeAccess(param.id, Ibos.app.url('weibo/dashboard/comment'), param, U.lang("WB.REMOVE_SELECT_COMMENTS_CONFIRM"));
		},
		// 恢复一条评论
		"recoverCm": function(param, elem) {
			WbDash.removeAccess(param.id, Ibos.app.url('weibo/dashboard/comment'), param, U.lang("WB.RECOVER_SELECT_COMMENTS_CONFIRM"));
		},
		// 删除选中话题
		"removeTps": function(param, elem) {
			var tpIds = U.getCheckedValue("topic");
			WbDash.removeAccess(tpIds, "#topic", param, U.lang("WB.REMOVE_SELECT_TOPICS_CONFIRM"));
		},
		// 删除一条话题
